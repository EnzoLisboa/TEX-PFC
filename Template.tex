...
\chapter*{\vspace{-14mm} Modelo ({\it template}) COEEL:  Utilizando a classe ~``cls-PFC-COEEL.cls''}

\vspace{-5mm}
Modelo (Template) ~\LaTeX ~da classe: {\bf cls-PFC-COEEL.cls} criada pelo professor Elvio, para atender às demandas de \textbf{PFC - Projeto Final de Curso} de Engenharia Elétrica (\gls{COEEL}) do IFBA {\it campus} Vitória da Conquista e \textbf{Relatórios Técnicos} de disciplinas e {\bf Pré-Projetos de PFC} de Engenharia Elétrica. {\bf Este é um modelo próprio e NÃO possui a intenção de seguir a norma ABNT-NBR14724} - Trabalhos Acadêmicos~\cite{NBR14724_TrabalhosAcademicos} e estaremos seguindo parcialmente a norma {ABNT-NBR6023} - Referências~\cite{NBR6023_Referencias}.
...
\section{Selecionar a classe para o modo de PFC, Relatório ou Pré-Projeto de PFC}

Este Modelo ({\it template}) pode ser utilizado para a produção de Monografias de {\bf PFC (Projeto Final de Curso)} de Egenharia Elétrica, para {\bf Relatórios} de disciplinas e {\bf Pré-Projetos de PFC}.
...
Para habilitar este modelo para funcionar como {\bf Pré-projeto de PFC}, altere o documento principal (Main) conforme mostra o código~\ref{cod:PreProjeto}: 

\ImportaCodigo[language=tex, caption=Para produzir Pré-Projetos de PFC, label=cod:PreProjeto]{./classe/arqcls/Cod_PreProjeto.txt}
...
{\bf Para gerar automaticamente a Capa e Folha de Rosto:} Entre no documento principal (\verb|a_PFC_main.tex|), e procure pelos comandos listados abaixo, perceba que existem locais apropriados para preenchimento caso seja um PFC, Pré-Projeto de PFC ou Relatório de Disciplinas:
...
\begin{itemize}
    ...
    \item {\Large \verb|\Orientador{}|:} preencha somente se for PFC ou Pré-Projeto de PFC;
    \item {\Large \verb|\Coorieantador{}|:} preencha somente se for PFC ou Pré-Projeto de PFC;
...
A ficha catalográfica é um serviço prestado pela Biblioteca, e {\bf neste modelo (template) a Ficha Catalográfica está disponível apenas para PFC.}
...
Para acrescentar automaticamente o PDF da Ficha Catalográfica fornecida pela Biblioteca no seu documento, abra a pasta  {\bf PFC\_Específicos} e procure pelo documento PDF denominado {\bf Ficha Catalográfica}, e substitua este arquivo pela sua ficha catalográfica enviada pela biblioteca.

\begin{CaixaVermelha}
    O seu PDF correspondente à Ficha Catalográfica deverá possuir o mesmo nome do arquivo do template, sem acentuação:\\

    {\bf Ficha\_Catalografica.pdf}
\end{CaixaVermelha}

Faça upload do PDF da sua Ficha Catalográfica no ~\LaTeX~Overleaf~ e substitua o arquivo PDF chamado: \verb|Ficha_Catalografica.pdf| pela sua Ficha Catalográfica.

Desta forma o ~\LaTeX~ irá importar e adicionar sua Ficha Catalográfica automaticamente no seu documento.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Adicionando automaticamente o PDF da Folha de Aprovação}

{\bf Neste modelo (template) a Folha de Aprovação está disponível apenas para PFC.}

Verifique o documento chamado \verb|b_FolhaAprovacao.tex|, que se encontra dentro da pasta PFC\_Específicos. 

Caso a Folha de Aprovação de seu trabalho precise ser assinada manualmente, à caneta, preencha o nome, titulação e campus dos membros da banca, imprima esta folha PDF e a leve no dia de sua defesa para que a banca possa assiná-la à caneta, depois você irá escaneá-la e adicioná-la da mesma forma que se adiciona a Folha de Aprovação assinada digitalmente, no documento \verb|b_FolhaAprovacao.tex|, conforme mostra o código~\ref{cod:FolhaAprovacao}.

Caso a Folha de Aprovação seja assinada digitalmente pelo sistema SEI ou pelo SOU.GOV, seu orientador irá providenciar a confecção da Folha de Aprovação e providenciará a assinatura dos membros da banca e você irá adicioná-la ao seu trabalho, no documento \verb|b_FolhaAprovacao.tex| conforme mostra o código~\ref{cod:FolhaAprovacao}. Renomeie o PDF da sua Folha de Aprovação assinada para \verb|Folha_Aprovacao.pdf| e faça o upload do PDF para o ~\LaTeX~Overleaf. Lembrando que o seu documento PDF deverá estar dentro da pasta {\bf PFC\_Especificos}.

%\pagebreak

\begin{Codigo}[language=tex, caption=Código para acrescentar PDF da Folha de Aprovação, label=cod:FolhaAprovacao]
%
\includepdf[pages=1]{./PFC_Especificos/Folha_Aprovacao.pdf} 
%
\end{Codigo}
...
{\bf Neste modelo (template) o Glossário está disponível apenas para PFC e Pré-Projeto de PFC.}

Verifique que existe um documento chamado: \verb|GLOSSARIO.tex|, abra este documento e verifique que este documento é uma Biblioteca de Lista de Siglas e Símbolos. Preencher este documento com as Siglas e com os Símbolos utilizados no seu documento.
...
\begin{Codigo}[language=tex, caption=Biblioteca de Glossários~ {\bf GLOSSARIO.TEX}, label=cod:Glo1]
   \newglossaryentry{apelido} 
    {
    name = {Nome que aparece no texto},
    description = {Descrição da sigla ou símbolo}
    }
\end{Codigo}

Vamos dar um exemplo de como preencher o \verb|GLOSSARIO.tex| e como citar no texto as siglas e símbolos. Supondo que queremos colocar no glossário a sigla IEEE e que sua descrição seja: IEEE - Instituto de Engenheiros Eletricistas e Eletrônicos (IEEE is the world's largest technical professional organization dedicated to advancing technology for the benefit of humanity) e queremos citar a sigla $cos(\varphi)$ com descrição de {Fator de Potência} podemos adicionar o seguinte código no documento \verb|GLOSSARIO.tex|, como mostra o código~\ref{cod:Glo2}:

\pagebreak

\begin{Codigo}[language=tex, caption=Biblioteca de Glossários~ {\bf GLOSSARIO.tex}, label=cod:Glo2]
    % para SIGLAS
...
\section{Documento ~~0\_CTRL+C.tex}

Este documento \verb|0_CTRL+C.tex| foi criado para facilitar a utilização dos comandos mais comuns a serem utilizados em seus Relatórios e Trabalhos Acadêmicos.

Utilize o documento \verb|0_CTRL+C.tex| para CTRL+C e CTRL+V em seu trabalho.

No documento \verb|0_CTRL+C.tex| você encontrará os seguintes comandos:

\begin{itemize}
    \item Figuras Simples;
    \item Duas Figuras Lado a Lado;
    \item Tabela Simples;
    \item Tabela Colorida;
    \item Equações Simples;
    \item Legenda de Equações com Tabulação;
    \item Equações Alinhadas em Várias Linhas;
    \item Legendas com Tabulação e Glossário;
    \item Caixa Verde e Caixa Vermelha;
    \item Códigos - Importar Códigos de Arquivos;
    \item Códigos Diretamente no Texto; 
\end{itemize}

Acrescente mais comandos no documento \verb|0_CTRL+C.tex| para facilitar a elaboração de seu PFC, Pré-Projeto de PFC ou Relatório de Disciplina.
...
{\bf Neste modelo (template) o Índice Remissivo está disponível apenas para PFC.}
...
Verifique que o Índice Remissivo é criado na última página de seu trabalho, possui separação das palavras em ordem alfabética contendo o número da página onde se encontra a palavra.
...
Trabalhos de Engenharia Elétrica, Automação, Computação e Robótica possuem projetos contendo a criação de softwares, programação de hardware, etc. Nesse sentido, é extremamente importante que o ~\LaTeX~ nos ajude a acrescentar códigos ou trechos de código de forma profissional em nosso texto. 
...
\begin{Codigo}[language=Python, caption=Programa em Python para Somar Dois Números, label=cod:CodPython]
    # exemplo de código linguagem  Python para
    # somar dois números
...
\vspace{5mm}
Para que esta função colora as palavras reservadas de linguagens de programação você precisa definir em qual linguagem (\verb|language=|) seu código foi escrito. Segue na tabela~\ref{tab:LinguagensSuportadas} a lista de linguagens suportadas:

\vspace{3mm}
\begin{table}[htb!]
   \centering
   \caption{Lista de Linguagens Suportadas para estes comandos de códigos}
   \begin{tabular}{llll} % l=Left(textos); r=Right(números)
      \Linha % linha grossa
ABAP      & Elan      & Miranda      & R      \\
ACSL      & erlang    & Mizar        & Reduce \\
Ada       & Euphoria  & ML           & Rexx   \\
Algol     & Fortran   & Modula-2     & RSL    \\
Ant       & GCL       & MuPAD        & Ruby    \\
Assembler & Gnuplot   & NASTRAN      & S       \\
Awk       & Haskell   & Oberon-2     & SAS     \\
bash      & HTML      & OCL          & {\bf Scilab}  \\
Basic     & IDL       & Octave       & sh      \\
{\bf C}   & inform    & Oz           & SHELXL  \\
{\bf C++} & {\bf Java}& Pascal       & Simula  \\
Caml      & JVMIS     & Perl         & SPARQL  \\
CIL       & ksh       & {\bf PHP}    & {\bf SQL}     \\
Clean     & Lingo     & PL/I         & tcl     \\
Cobol     & Lisp      & Plasm        & {\bf TeX} (\LaTeX) \\
Comal     & Logo      & PostScript~~~~~~ & VBScript   \\
command.com~~~~~~ & make & POV       & Verilog   \\
Comsol    & Mathematica~~~~~~ & Prolog & VHDL       \\
csh       & {\bf Matlab} & Promela   & VRML   \\
Delphi    & Mercury   & PSTricks     & XML    \\
Eiffel    & MetaPost  & {\bf Python} & XSLT \\
      \Linha % linha grossa
   \end{tabular}
   \FonteTab{\url{https://pt.overleaf.com/learn/latex/Code_listing}}
   \label{tab:LinguagensSuportadas}
\end{table}
...
\begin{Codigo}[language=TeX, 
 ...
 \begin{figure}[htb]
   \centering
   \includegraphics[width=6.5cm]{./arquivos/LogoIFBA-VDC-V.pdf}
   \FonteFig{Reitoria do IFBA}
   \caption{Logomarca horizontal do IFBA campus Vitória 
   da Conquista}
   \label{fig:LogoIFBAv}
 \end{figure}
\end{Codigo}
...
\begin{CaixaVermelha}
    Não se esqueça da {\bf fonte de referências das figuras}, é obrigatório citar onde foram retiradas as figuras, caso seja você mesmo quem as construiu, cite como: Próprio autor.
\end{CaixaVermelha}

\vspace{1cm}
Segue o código~\ref{cod:Figs2a2} na página~\pageref{cod:Figs2a2}, de como colocar 2 figuras lado a lado. Lembrando que o comando \verb|\fbox{}| é opcional.
...
Lembre-se que a qualidade das figuras é fundamental para que seu trabalho se torne profissional. Verifique a resolução das figuras e opte pelos formatos vetorizados. Nunca se esqueça de citar as fontes das figuras, porém o melhor é que você as construa (caso seja possível).

\begin{CaixaVerde}
    Caso você tenha que capturar alguma figura via {\bf Print Screen}, aumente o zoom da imagem de forma que ela ocupe toda a tela do computador, pois dessa forma a imagem terá um maior número de pixels, melhorando a qualidade da imagem importada em seu trabalho.
\end{CaixaVerde}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak
...
\begin{Codigo}[language=tex, 
    ...
    \begin{CaixaVerde}
        Exemplo de uma caixa verde com marcação inferior 
        na cor RGB verde (50,160,65) padrão dos Institutos 
        Federais.
    \end{CaixaVerde}
...
Para este template de PFC, Pré-Projeto de PFC e Relatório de Disciplina, foi criada uma biblioteca exemplo com o nome {\bf BIB\_pfc.bib}, que você pode encontrar na figura~\ref{fig:bib_pfc}. 
...
\begin{Codigo}[language=tex, 
  ...
  title =    "Máquina Revolucionária para Desempenar Bananas 
              Utilizando o Método Desimpenator Bananífico",
...
\subsection{Citações de Referências com os comandos: cite,  citeonline e CiteLiteral}
...
O comando {\bf CiteLiteral} foi criado para citar textos exatos, completos sem alterar nenhuma palavra do autor original.

Deve-se evitar o excesso de citações desta forma, uma vez que é mais bem visto quando você cita um determinado autor comentando com suas próprias palavras. 

Porém há casos em qua há necessidade de citar literalmente sem modificar nenhuma palavra, então temos o comando {\bf CiteLiteral}, exemplo:

\begin{CiteLiteral}{\cite{Buda}}
...
Verifique que o texto ficou recuado, em itálico, com aspas duplas e já acrescentou a fonte. Para utilizar o comando {CiteLiteral}, proceda como o código~\ref{cod:CiteLiteral}:

\begin{Codigo}[language=TeX, caption=Citações Literais, label=cod:CiteLiteral]
\begin{CiteLiteral}{\cite{Buda1}}
    Aqueles que dão importância àquilo que não é importante 
    e que não dão importância àquilo que é importante, 
    sustentando pensamentos errôneos, eles nunca alcançarão 
